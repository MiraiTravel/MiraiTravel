...
class QqBot extends MiraiTravelSoftware
{
    ...
    function send_friend_message($argc, $argv)
    {
        if ($argc === 3) {
            ...
            $messageChain = new $qqbot->adapterCore["messageChain"]();
    ...
    function send_group_message($argc, $argv)
    {
        if ($argc === 3) {
            ...
            $qqbot = $qqbotManager->get_qqobj($argv[0]);
            if (!$qqbot) {
                $this->logSystem->println("未挂载 " . $argc[0] . "的QQBot。", "Red");
                $this->logSystem->println("挂载 " . json_encode($qqbotManager::$qqObjArray) . "的QQBot。", "Red");
                return 0;
            }
            $messageChain = new $qqbot->adapterCore["messageChain"]();
            $messageChain->push_plain($argv[2]);
