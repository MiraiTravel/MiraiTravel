...
namespace MiraiTravel\Software\Stay\Connection;
...
class TcpConnection extends ConnectionInterface
{
     ...
     */
    public function __construct($socket, $remote_address = '' , $globalEvent)
    {
        ...
        $this->globalEvent->add($this->_socket, EventInterface::EV_READ, array($this, 'baseRead'));
    ...
    public function send($send_buffer, $raw = false)
    {
        ...
        if ($this->_sendBuffer === '') {
            if ($this->transport === 'ssl') {
                $this->globalEvent->add($this->_socket, EventInterface::EV_WRITE, array($this, 'baseWrite'));
                $this->_sendBuffer = $send_buffer;
                $this->checkBufferWillFull();
                return;
            }
    ...
    {
        $this->globalEvent->del($this->_socket, EventInterface::EV_READ);
        $this->_isPaused = true;
    }
    ...
    public function baseRead($socket, $check_eof = true)
    {
        ...
        if ($this->transport === 'ssl' && $this->_sslHandshakeCompleted !== true) {
            if ($this->doSslHandshake($socket)) {
                ...
                if ($this->_sendBuffer) {
                    //$this->globalEvent->add($socket, EventInterface::EV_WRITE, array($this, 'baseWrite'));
                }
            } else {
                return;
            }
    ...
    public function baseWrite()
    {
        ...
        if ($len === \strlen($this->_sendBuffer)) {
            $this->bytesWritten += $len;
            $this->globalEvent->del($this->_socket, EventInterface::EV_WRITE);
            $this->_sendBuffer = '';
